...
constexpr bool kRuntimeUartEnabled = !KUGLASS_STANDALONE_OUTPUT_TEST ||
    KUGLASS_STANDALONE_UART_ENABLED;
constexpr bool kRuntimeAnalogEnabled = !KUGLASS_STANDALONE_OUTPUT_TEST ||
    KUGLASS_STANDALONE_ANALOG_ENABLED;
constexpr bool kRuntimeStatusEnabled = kRuntimeUartEnabled;
constexpr bool kRuntimeSafetyInputsEnabled =
    !KUGLASS_STANDALONE_OUTPUT_TEST ||
    KUGLASS_STANDALONE_SAFETY_INPUTS_ENABLED;
...
uint32_t g_standalone_output_release_ms = 0;
...
bool commit_channel_enable_if_safe(size_t channel_index, void*) {
    ...
    if constexpr (!kRuntimeSafetyInputsEnabled) {
        return gpio_set_level(static_cast<gpio_num_t>(
...
bool physical_safety_inputs_high() {
    if constexpr (!kRuntimeSafetyInputsEnabled) return true;
...
bool estop_is_active() {
    if constexpr (!kRuntimeSafetyInputsEnabled) return false;
...
bool configure_standalone_test_command() {
    ProtocolCommand command = {};
    command.seq = 1U;
    // The command is generated locally and has no communication lease. Using
    // the full u32 interval keeps FaultManager's physical-input priority and
    // latch behavior without manufacturing a 1 ms heartbeat.
    command.ttl_ms = UINT32_MAX;
    command.channel_count = KUGLASS_CHANNEL_COUNT;
    for (size_t i = 0; i < KUGLASS_CHANNEL_COUNT; ++i) {
        command.channels[i] = ProtocolChannelCommand{
            static_cast<uint8_t>(i), KUGLASS_STANDALONE_TEST_MI, true};
    }
    if (!g_channels.apply_command(command)) {
        g_fault.reject_command();
        return false;
...
bool standalone_output_released(uint32_t now_ms) {
    return static_cast<int32_t>(now_ms - g_standalone_output_release_ms) >= 0;
}
...
void output_task(void*) {
    ...
    while (true) {
        ...
        xSemaphoreTake(g_state_mutex, portMAX_DELAY);
        const bool command_ok = !KUGLASS_STANDALONE_OUTPUT_TEST ||
            standalone_output_released(now_ms);
        bool estop_ok = false;
        const bool stages_ok = inputs_ok(&estop_ok);
        if constexpr (kRuntimeSafetyInputsEnabled) {
            g_fault.update(now_ms, estop_ok, stages_ok);
        }
        g_channels.update(dt_s, command_ok && !g_fault.faulted());

        const SafetyEventCounters before_output = safety_event_snapshot();
        const bool pending_before_output =
            !command_ok || safety_events_pending(before_output) ||
            !stages_ok || !estop_ok;
...
bool configure_output_watchdog() {
    ...
    config.timeout_ms = KUGLASS_STANDALONE_OUTPUT_TEST
        ? KUGLASS_STANDALONE_WATCHDOG_MS
...
bool create_runtime_tasks() {
    bool created =
        xTaskCreate(output_task, "spwm", 4096, nullptr, 9, nullptr) == pdPASS;
    if constexpr (kRuntimeUartEnabled) {
        created = created &&
            xTaskCreate(rx_task, "a_rx", 4096, nullptr, 8, nullptr) == pdPASS;
    }
    if constexpr (kRuntimeAnalogEnabled) {
        created = created &&
            xTaskCreate(analog_task, "analog", 4096, nullptr, 4, nullptr) ==
                pdPASS;
    }
    if constexpr (kRuntimeStatusEnabled) {
        created = created &&
...
extern "C" void app_main(void) {
    ...
    if constexpr (KUGLASS_STANDALONE_OUTPUT_TEST) {
        if (!configure_standalone_test_command()) {
            g_spwm.force_safe();
            return;
        }
        g_standalone_output_release_ms = millis_now() +
            KUGLASS_STANDALONE_STARTUP_SETTLE_MS;
    }
    ...
    if constexpr (kRuntimeAnalogEnabled || kRuntimeStatusEnabled) {
    ...
    if (g_state_mutex == nullptr || g_task_start_event == nullptr ||
        (kRuntimeUartEnabled && g_uart_mutex == nullptr) ||
        ((kRuntimeAnalogEnabled || kRuntimeStatusEnabled) &&
         g_analog_mutex == nullptr) ||
        (kRuntimeStatusEnabled && g_status_tx_mutex == nullptr) ||
