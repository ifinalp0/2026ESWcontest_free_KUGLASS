...
이 폴더는 `ESP32_B_Algo`의 출력 알고리즘을 사용하되, 미연결 입력과 진단
부하가 파형을 끊지 않도록 구성한 **안정 운전 우선 standalone bench build**입니다.
ESP32_A 명령 없이 내부 MI를 사용하지만, Logic Carrier의 `EN_GLOBAL`과 네
`FAULT_N` 안전 입력은 실제 배선 상태를 그대로 감시합니다.

- 부팅 시 CH0~CH3 모두 `MI=0.75`, `enable=true`인 내부 명령을 한 번만
  transaction으로 적용합니다. 1 ms 루프에서 명령을 재작성하거나 TTL을
  갱신하지 않습니다.
- `ENABLE LOW`와 PWM force-low 상태로 250 ms를 기다린 뒤 기존 MI slew로
  출력이 부드럽게 상승합니다.
- UART1, UART RX/status task와 ADC task는 기본적으로 만들지 않습니다. 따라서
  floating RX의 손상 frame, 주기 status 송신, 미연결 ADC scan이 출력 루프에
  개입하지 않습니다.
- `EN_GLOBAL`과 CH0~CH3 `FAULT_N`은 production과 같은 GPIO mode와
  falling-edge ISR로 감시합니다. 어느 하나라도 LOW이거나 LOW edge가 latch되면
  네 raw enable과 PWM을 즉시 safe-off합니다.
...
핀맵, 16 kHz/60 Hz SPWM, 최대 MI 0.95, 방향 전환 시 `ENABLE LOW + PWM LOW`
1 ms blanking과 hard safe-off 순서는 유지합니다. TEST 전용 task watchdog은
probe/debugger의 짧은 정지를 nuisance reset으로 만들지 않도록 500 ms이며,
실제 timeout 때는 ISR 경로로 raw enable을 LOW로 내립니다.

## 안정 운전 프로파일

| 항목 | TEST 기본값 | 안정성 효과 |
| --- | --- | --- |
| 내부 명령 | 부팅 시 1회 적용 | 1 ms 공유 상태 재작성 제거 |
| 출력 시작 | 250 ms safe settle | 전원·GPIO 초기 과도 구간 분리 |
| 런타임 task | `spwm`만 생성 | UART/ADC/status 부하와 jitter 제거 |
| UART | 초기화하지 않음 | floating RX와 bridge traffic 영향 제거 |
| ADC | 초기화하지 않음 | 미연결 입력 scan과 lock 경합 제거 |
| 안전 입력 | `EN_GLOBAL`/`FAULT_N` 활성 | 물리 LOW와 falling edge 즉시 safe-off |
| watchdog | 500 ms, panic/safe-off 유지 | bench 지연에 의한 false trip 감소 |

물리 Fault는 latch되며 입력이 다시 HIGH가 되어도 자동 복구하지 않습니다.
standalone 기본값은 UART/reset command를 비활성화하므로 E-Stop 또는
`FAULT_N` trip 뒤에는 안전 원인을 제거한 다음 보드를 재부팅해야 합니다.
이 프로파일은 A→B TTL, 상태 회신과 ADC를 제외하므로 `ESP32_B_Algo`의 대체
펌웨어가 아닙니다.
...
- 출력 task는 ENABLE commit 전후에 모든 안전 입력과 ISR event를 다시 확인합니다.
- task watchdog 실패는 `ENABLE LOW + PWM force-low + applied_mi=0`으로 처리합니다.
...
- 출력 task는 실제 경과시간으로 60 Hz 위상과 MI slew를 갱신하며 TEST 전용 500 ms task watchdog에 등록됩니다.

물리 E-Stop과 Power Stage의 `RUN_OK` 차단이 최종 안전 경로입니다. 이 펌웨어만으로 인증된 안전 회로를 대체하지 않습니다.

## 비활성 통신/reset 코드 참고

다음 UART 명령과 reset 코드는 공유 source에 남아 있지만 standalone 기본
프로파일에서는 UART와 RX task를 만들지 않으므로 실행 경로에 들어오지 않습니다.
...
다음 status와 ADC 코드는 공유 source에 남아 있지만 standalone 기본
프로파일에서는 ADC, UART, analog/status task를 만들지 않습니다.
...
공유 코드의 UART1 기본 핀은 TX=GPIO43, RX=GPIO44, 115200 8-N-1이지만 이
TEST 기본 프로파일에서는 UART peripheral을 초기화하지 않습니다. UART 진단을
명시적으로 다시 켤 경우 Logic Carrier에는 A↔B connector가 없으므로 공통
GND를 포함한 외부 harness가 필요합니다.
