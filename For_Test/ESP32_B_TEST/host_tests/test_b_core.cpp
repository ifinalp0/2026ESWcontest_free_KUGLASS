...
int main() {
    ...
    assert(KUGLASS_STANDALONE_OUTPUT_TEST);
    assert(near(KUGLASS_STANDALONE_TEST_MI, 0.75f));
    assert(KUGLASS_STANDALONE_SAFETY_INPUTS_ENABLED);
    assert(!KUGLASS_STANDALONE_UART_ENABLED);
    assert(!KUGLASS_STANDALONE_ANALOG_ENABLED);
    assert(KUGLASS_STANDALONE_STARTUP_SETTLE_MS == 250U);
    assert(KUGLASS_STANDALONE_WATCHDOG_MS == 500U);
    assert(KUGLASS_STANDALONE_WATCHDOG_MS > KUGLASS_SAFETY_WATCHDOG_MS);
