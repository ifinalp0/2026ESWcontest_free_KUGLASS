...
void IRAM_ATTR apply_fixed_enable_levels_from_isr() {
    if (!g_safety_outputs_ready) return;
    for (size_t i = 0; i < KUGLASS_CHANNEL_COUNT; ++i) {
        (void)gpio_set_level(
            static_cast<gpio_num_t>(g_enable_gpios[i]),
...
void IRAM_ATTR begin_safety_trip_from_isr() {
    ...
    __asm__ __volatile__("memw" ::: "memory");
    apply_fixed_enable_levels_from_isr();
}
...
void IRAM_ATTR safety_falling_edge_isr(void* argument) {
    ...
    // Re-apply the fixed HIGH test levels before releasing the lock. PWM is
    // forced low by the output task after the trip becomes visible.
    apply_fixed_enable_levels_from_isr();
...
bool commit_channel_enable_if_safe(size_t channel_index, void*) {
    ...
    if (safe) {
        safe = gpio_set_level(static_cast<gpio_num_t>(
            ...
            1) == ESP_OK;
        __asm__ __volatile__("memw" ::: "memory");
        // Preserve the original trip/commit race check even though this test
        // variant deliberately re-applies HIGH to the raw ENABLE GPIO.
        if (g_safety_trip_latched || g_safety_trip_inflight != 0U ||
        ...
        if (!safe) apply_fixed_enable_levels_from_isr();
    }
    portEXIT_CRITICAL(&g_safety_event_mux);
    return safe;
}
...
bool physical_safety_inputs_high() {
    if constexpr (KUGLASS_STANDALONE_OUTPUT_TEST) return true;
...
bool estop_is_active() {
    if constexpr (KUGLASS_STANDALONE_OUTPUT_TEST) return false;
...
bool refresh_standalone_test_command(uint32_t now_ms) {
    ProtocolCommand command = {};
    command.seq = 1U;
    command.ttl_ms = KUGLASS_DEFAULT_TTL_MS;
...
void output_task(void*) {
    ...
    while (true) {
        ...
        if constexpr (KUGLASS_STANDALONE_OUTPUT_TEST) {
            command_ok = refresh_standalone_test_command(now_ms);
        }
        bool estop_ok = false;
        const bool stages_ok = inputs_ok(&estop_ok);
        ...
        if (pending_before_output) {
            ...
            // If an ISR ran while outputs were being updated, keep ENABLE HIGH
            // for this test variant and force PWM low before leaving the cycle.
...
bool configure_safety_inputs() {
    // Deliberately leave the disconnected sense pins untouched in the
    // standalone test. No pull direction or floating level can inhibit the
    // raw ESP32_B output pins in this build.
    if constexpr (KUGLASS_STANDALONE_OUTPUT_TEST) return true;
...
#ifdef ESP_PLATFORM
    // This is intentionally the first peripheral action: drive the raw MCU
    // enables HIGH for this test variant and hold every PWM output low before
    // allocation/comms/tasks.
