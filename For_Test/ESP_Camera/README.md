...
> **저장소 역할:** `For_Test/ESP_Camera/`는 standalone 카메라 시험 프로젝트입니다. 제품용 `ESP32_A_Algo/`는 이 폴더의 코드·설정·빌드 캐시에 의존하지 않습니다.
...
이 폴더는 제품 PDLC 제어 펌웨어와 분리된 ESP-IDF 프로젝트입니다. 저장소 루트에서
`For_Test/ESP_Camera`로 이동한 뒤 명령을 실행합니다. 프로젝트 CMake 설정과 컴포넌트
...
이 폴더의 firmware는 카메라 단독 시험용이며 `ESP32_A_Algo`에 직접 링크되지
않습니다. `ESP32_A_Algo`는 검증된 GPIO4~18 프로필과 OV2640 RGB565 capture
계약을 자체 `main/camera_service.*`, `main/camera_pins.h`로 소유하며 카메라와
알고리즘을 같은 ESP32-S3에서 실행합니다.

따라서 이 standalone 프로젝트의 경로·빌드 캐시 변경은 ESP32_A 빌드에
영향을 주지 않습니다. 양쪽 구현을 변경할 때는 자동 공유를 전제로 하지
말고 핀맵과 capture 계약의 차이를 명시적으로 비교·반영합니다. standalone
시험과 관련 자료를 보존할 필요가 없다면 이 폴더는 삭제할 수 있습니다.
