...
> **저장소 역할:** `ESP_Camera/`는 선택적으로 보존하는 standalone 카메라 시험 레퍼런스입니다. 제품용 `ESP32_A_Algo/`는 이 폴더의 코드·설정·빌드 캐시에 의존하지 않습니다. 독립 시험 자료가 더 이상 필요하지 않다면 이 폴더 전체를 삭제해도 ESP32_A의 빌드·플래시에는 영향이 없습니다.
