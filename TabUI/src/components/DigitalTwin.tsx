...
// The GLB uses +X for vehicle-left and -X for vehicle-right.
...
const unknownFilmColor = new Color('#8c9ba5');
...
function applyFilmVisuals(
  ...
  if (!appliedKnown) {
    if (filmMaterial) {
      filmMaterial.color.copy(unknownFilmColor);
      filmMaterial.opacity = 0.12 + (selected ? 0.06 : 0);
      filmMaterial.roughness = 0.46;
      filmMaterial.transmission = 0.26;
      filmMaterial.thickness = 0.04;
      filmMaterial.clearcoat = 0.35;
      filmMaterial.clearcoatRoughness = 0.42;
    ...
    if (hazeMaterial) {
      hazeMaterial.opacity = selected ? 0.045 : 0.02;
    }
    if (edgeMaterial) {
      edgeMaterial.color.copy(fault ? faultEdgeColor : selected ? activeEdgeColor : passiveEdgeColor);
      edgeMaterial.opacity = fault ? 0.96 : selected ? 0.86 : 0.28;
    }
    return;
  }
