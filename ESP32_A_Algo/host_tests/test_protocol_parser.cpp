...
int main() {
    ...
    assert(std::fabs(command.channels[0].mi -
                     KUGLASS_MAX_MODULATION_INDEX) < 0.001f);
    ...
    for (size_t i = 0; i < outbound.channel_count; ++i) {
        ...
        outbound.channels[i].mi =
            KUGLASS_MAX_MODULATION_INDEX * static_cast<float>(i) / 3.0f;
    ...
    assert(std::fabs(round_trip.channels[3].mi -
