...
열부하 시나리오는 기본적으로 DS18B20 내부온도를 사용한다. MOCK 또는 양쪽에서
명시적으로 허용한 HIL에서는 TabUI의 외부 온도 시연값을 임시 thermal policy
입력으로 사용할 수 있지만 production 센서가 추가된 것은 아니다. 시연 입력을
해제하거나 시나리오를 바꾸면 물리 센서 입력으로 복귀한다. 차박과 주차는 전 채널에
`enable=false`, MI 0.0을 보내 ESP32_B의 hard safe-off로 PDLC 출력을 완전히 끈다.
이는 채널 출력 차단이며 전원 rail 전체 차단을 의미하지 않는다.
